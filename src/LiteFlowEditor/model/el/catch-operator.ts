...
import IntermediateErrorBoundaryIcon from 'liteflow-editor-client/LiteFlowEditor/assets/intermediate-event-catch-error.svg';
import {
  ConditionTypeEnum,
  LITEFLOW_EDGE,
  ...
  public toCells(options: Record<string, any> = {}): Cell[] {
    ...
    if (!this.collapsed) {
      ...
      [condition, ...children].forEach(
        (item: ELNode | undefined, index: number) => {
          const next =
            item || NodeOperator.create(this, NodeTypeEnum.VIRTUAL, ' ');
